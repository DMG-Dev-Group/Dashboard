...
export function EquipeScene({ members }: Props) {
  ...
  if (!ready) return <div className="h-[900px]" />;
  ...
  return (
    <div className="space-y-6">
      ...
      <DmgHeader reducedMotion={reducedMotion} />
...
/** Cabeçalho decorativo — as mesmas partículas com "vento" da seção antiga, compactas, só como assinatura visual. */
function DmgHeader({ reducedMotion }: { reducedMotion: boolean }) {
  return (
    <div className="relative h-[220px] w-full overflow-hidden rounded-2xl border border-white/10 bg-[radial-gradient(120%_140%_at_50%_10%,rgba(192,24,26,.16),transparent_65%),#050505]">
      <SceneErrorBoundary fallback={null}>
        <Canvas dpr={[1, 1.8]} camera={{ position: [0, 0, 6.6], fov: 30 }} gl={{ antialias: true, alpha: true }}>
          <ambientLight intensity={0.4} />
          <pointLight position={[0, 1, 4]} color="#ff4d4f" intensity={1.4} distance={10} />
          <DmgHeaderContent reducedMotion={reducedMotion} />
        </Canvas>
      </SceneErrorBoundary>
    </div>
  );
...
function DmgHeaderContent({ reducedMotion }: { reducedMotion: boolean }) {
  const { viewport } = useThree();
  const positions = useMemo<[number, number, number][]>(() => {
    const gap = THREE.MathUtils.clamp(viewport.width * 0.24, 1.35, 1.75);
    return [-gap, 0, gap].map((x) => [x, 0, 0] as [number, number, number]);
  }, [viewport.width]);
